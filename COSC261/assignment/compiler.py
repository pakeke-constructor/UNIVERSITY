...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        re.match("\\s", self.input_string[self.current_char_index]):
    ...
    def get_token(self):
        ...
        # checking if we have finished scanning.
    ...
    def consume(self, *expected_tokens):
        ...
        if not (tok[0] in expected_tokens):
            self.unexpected_token(tok[0], expected_tokens)
        
        self.current_token = self.get_token()

        if tok[0] in [Token.NUM, Token.ID]:
            return tok
        else:
            return tok[0]
...
class Token:
    ...
    # Just some checks so i dont overwrite accidentally
    tbuffer = set()
    for (tok, patt) in token_regexp:
        assert (tok not in tbuffer), "Duplicate token: " + tok
        tbuffer.add(tok)
    del tbuffer
...
class If_Else_AST:
    ...
    def __repr__(self):
        return 'if ' + repr(self.condition) + ' then ' + \
                repr(self.then) + ' else ' + repr(self.elsethen) + ' end '

    def indented(self, level):
        return indent("If-Else", level) + self.cond.indented( level + 1) \
...
def if_statement():
    ...
    then = statements()
    tok_type = scanner.consume(Token.END, Token.ELSE)
    if tok_type == Token.END:
        return If_AST(condition, then)
    else: # tok_type is ELSE:
        else_block = statements()
        scanner.consume(Token.END) # It consumed an ELSE before, so we gotta consume an END.
        return If_Else_AST(condition, then, else_block)
